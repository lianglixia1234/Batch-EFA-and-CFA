...
def _build_merged_df_for_group(measure_names: List[str], available_measures: Dict[str, Dict]) -> Optional[pd.DataFrame]:
    ...
    for m_name in measure_names:
        if m_name not in available_measures:
            continue
        df_m = available_measures[m_name]['df'].copy()
        if n_rows is None:
            n_rows = len(df_m)
        elif len(df_m) != n_rows:
            # 行数不一致时降级为内连接（按索引）
            pass
        prefix = _sanitize_name(m_name) + "__"
        rename_map = {c: prefix + _sanitize_name(c) for c in df_m.columns}
        df_m_renamed = df_m.rename(columns=rename_map)
        if base_df is None:
            base_df = df_m_renamed.copy()
        else:
            # 若行数一致则直接赋值；若不一致用 concat 内连接
            if len(df_m_renamed) == len(base_df):
                for c in df_m_renamed.columns:
                    if c not in base_df.columns:
                        base_df[c] = df_m_renamed[c].values
            else:
                base_df = pd.concat([base_df, df_m_renamed], axis=1, join="inner")
    return base_df


def _run_multifactor_and_extract(
    df_merged: pd.DataFrame,
    measure_names: List[str],
    items_map: Dict[str, List[str]],
    method_items_raw: List[str],
):
    """
    根据当前各 measure 的题目映射，构建带前缀数据并运行 multi-factor CFA。
    返回统一格式的结果字典。
    """
    all_prefixed_cols = []
    ...
    for m in measure_names:
        ...
        for item in items_map.get(m, []):
            safe_item = _sanitize_name(item)
            col_name = prefix + safe_item
            if col_name in df_merged.columns:
                all_prefixed_cols.append(col_name)
                safe_map[item] = safe_item

    for item in method_items_raw:
        for m in measure_names:
            prefix = _sanitize_name(m) + "__"
            safe_item = _sanitize_name(item)
            col_name = prefix + safe_item
            if col_name in df_merged.columns and col_name not in prefixed_method_items:
                prefixed_method_items.append(col_name)
                break

    prefixed_method_items = list(dict.fromkeys(prefixed_method_items))
    used_cols = list(dict.fromkeys(all_prefixed_cols + prefixed_method_items))

    if not used_cols:
        return None, "无有效列可用于建模。"

    df_fit = df_merged[used_cols].copy()
    df_fit = df_fit.apply(pd.to_numeric, errors="coerce")
    bad_cols = [c for c in df_fit.columns if df_fit[c].isna().all()]
    if bad_cols:
        return None, f"以下列无法转换为数值：{', '.join(bad_cols[:8])}"
    df_fit = df_fit.dropna(axis=0, how="any")
    df_fit = df_fit.replace([np.inf, -np.inf], np.nan).dropna()

    if df_fit.empty or len(df_fit) < 10:
        return None, f"有效样本不足（{len(df_fit)} < 10）。"

    model, estimates, stats_dict, err, syntax = run_multifactor_cfa(
        df_fit, measure_names, prefixed_method_items
    )
    if err:
        return None, err
    ...
    measure_n_items = {m: len(items_map.get(m, [])) for m in measure_names}
    ...
    return {
        ...
        'rmsea': rmsea,
        'srmr': srmr,
        'total_n_items': sum(measure_n_items.values()),
        'measure_n_items': measure_n_items,
        'items_map': {m: list(v) for m, v in items_map.items()},
        'method_items': list(method_items_raw),
        'df_fit': df_fit,
        'prefixed_method_items': prefixed_method_items,
    }, None


def _try_all_deletions_multifactor(
    df_merged: pd.DataFrame,
    measure_names: List[str],
    items_map: Dict[str, List[str]],
    method_items: List[str],
    N_multi_map: Dict[str, int],
    n_multi_map: Dict[str, int],
):
    """
    尝试删除每一个题目（按 measure 逐个尝试），受 N_multi / n_multi 约束。
    """
    results = []
    for m_name in measure_names:
        current_items = items_map.get(m_name, [])
        for item in current_items:
            # 约束1：measure 总题数必须 > N_multi（底线）
            if len(current_items) <= N_multi_map.get(m_name, 3):
                continue

            # 约束2：若该题是反向题，删除后反向题数必须 > n_multi
            if _is_reverse_method_item(item):
                rev_count = len([x for x in current_items if _is_reverse_method_item(x)])
                if rev_count <= n_multi_map.get(m_name, 0):
                    continue

            new_items_map = {m: list(v) for m, v in items_map.items()}
            new_items_map[m_name] = [x for x in new_items_map[m_name] if x != item]
            new_method = [x for x in method_items if x != item]

            res, err = _run_multifactor_and_extract(df_merged, measure_names, new_items_map, new_method)
            if err:
                continue
            res['deleted_item'] = item
            res['deleted_measure'] = m_name
            results.append(res)
    return results


def _score_multifactor_stage1(r, current_cfi, current_tli):
    """Stage 1：保质量（0.90），不追求过度精简"""
    cfi, tli = r['cfi'], r['tli']
    if np.isnan(cfi) or np.isnan(tli):
        return -9999
    if cfi >= 0.90 and tli >= 0.90:
        return 10000 + cfi * 100 + tli * 100 - r['total_n_items'] * 10
...
def _score_multifactor_stage2(r, current_cfi, current_tli):
    """Stage 2：求精简（0.95），不能破坏 0.90 底线"""
    cfi, tli = r['cfi'], r['tli']
    if np.isnan(cfi) or np.isnan(tli):
        return -9999
    ...
    if cfi >= 0.95 and tli >= 0.95:
        return 10000 + cfi * 100 + tli * 100 - r['total_n_items'] * 50
...
def run_auto_multifactor_cfa_two_stages(
    df_merged: pd.DataFrame,
    measure_names: List[str],
    initial_items_map: Dict[str, List[str]],
    initial_method_items: List[str],
    N_multi_map: Dict[str, int],
    n_multi_map: Dict[str, int],
    max_rounds: int = 30,
):
    """
    两阶段自动删题 Multi-Factor CFA。
    返回结构与 single 批量模式一致。
    """
    # ==================== Stage 1: 保质量 ====================
    s1_items_map = {m: list(v) for m, v in initial_items_map.items()}
    s1_method = list(initial_method_items)
    s1_history = []

    res, err = _run_multifactor_and_extract(df_merged, measure_names, s1_items_map, s1_method)
    if err:
        return {'stage1': None, 'stage2': None, 'error': f"Stage 1 初始模型失败: {err}"}

    s1_history.append({
        'round': 0, 'action': '初始模型（全题目）', 'deleted_item': None, 'deleted_measure': None,
        'items_map': {m: list(v) for m, v in s1_items_map.items()},
        'measure_n_items': res['measure_n_items'], 'total_n_items': res['total_n_items'],
    ...
    if not (res['cfi'] >= 0.90 and res['tli'] >= 0.90):
        ...
        while round_num <= max_rounds:
            test_results = _try_all_deletions_multifactor(
                df_merged, measure_names, s1_items_map, s1_method, N_multi_map, n_multi_map
            )
            if not test_results:
                break
            for r in test_results:
                r['score'] = _score_multifactor_stage1(r, res['cfi'], res['tli'])
            ...
            if best['score'] < 0 and (res['cfi'] < 0.90 or res['tli'] < 0.90):
                break
            s1_items_map = best['items_map']
            s1_method = best['method_items']
            res = best
            s1_history.append({
                'round': round_num, 'action': f'删除 {best["deleted_item"]}（来自 {best["deleted_measure"]}）',
                'deleted_item': best['deleted_item'], 'deleted_measure': best['deleted_measure'],
                'items_map': {m: list(v) for m, v in s1_items_map.items()},
                'measure_n_items': best['measure_n_items'], 'total_n_items': best['total_n_items'],
                'cfi': best['cfi'], 'tli': best['tli'], 'rmsea': best['rmsea'], 'srmr': best['srmr'],
                'result': best['result'], 'estimates': best['estimates'], 'syntax': best['syntax'],
                'df_fit': best['df_fit'],
            })
    ...
    # ==================== Stage 2: 求精简 ====================
    s2_items_map = {m: list(v) for m, v in stage1_final['items_map'].items()}
    s2_method = list(stage1_final['method_items'])
    s2_history = []

    res2, err2 = _run_multifactor_and_extract(df_merged, measure_names, s2_items_map, s2_method)
    if err2:
        return {
    ...
    s2_history.append({
        'round': 0, 'action': '承接 Stage 1 最终题目', 'deleted_item': None, 'deleted_measure': None,
        'items_map': {m: list(v) for m, v in s2_items_map.items()},
        'measure_n_items': res2['measure_n_items'], 'total_n_items': res2['total_n_items'],
    ...
    if not already_high:
        ...
        while round_num <= max_rounds:
            test_results = _try_all_deletions_multifactor(
                df_merged, measure_names, s2_items_map, s2_method, N_multi_map, n_multi_map
            )
            if not test_results:
                break
            for r in test_results:
                r['score'] = _score_multifactor_stage2(r, res2['cfi'], res2['tli'])
            valid = [r for r in test_results if r['score'] > -1000]
            if not valid:
                break
            valid.sort(key=lambda x: x['score'], reverse=True)
            ...
            if best['score'] < 0:
                break
            s2_items_map = best['items_map']
            s2_method = best['method_items']
            res2 = best
            s2_history.append({
                'round': round_num, 'action': f'删除 {best["deleted_item"]}（来自 {best["deleted_measure"]}）',
                'deleted_item': best['deleted_item'], 'deleted_measure': best['deleted_measure'],
            ...
            if best['cfi'] + best['tli'] > best_score2:
                ...
                best_overall2 = best
            if best['cfi'] >= 0.95 and best['tli'] >= 0.95:
                break
            round_num += 1
...
def render_batch_multi_cfa():
    st.markdown("""
    本模块利用 **Single-Factor CFA 已确认的题目**，批量运行 **Multi-Factor Correlated Model** 的自动删题 CFA。
    - 每个 Measure Group 包含多个 Measure（潜变量）
    - *Stage 1（保质量）*：保证全局 CFI ≥ 0.90、TLI ≥ 0.90，未达标自动删题
    - *Stage 2（求精简）*：追求全局 CFI ≥ 0.95、TLI ≥ 0.95
    - 每个 Measure 受独立的 **N_multi / n_multi** 约束保护，达到底线后禁止继续删除该维度题目
    """)

    # ==========================================================
    # 1. 检查 Single CFA 确认结果
    # ==========================================================
    confirmed_measures = st.session_state.get("batch_confirmed", {})
    if not confirmed_measures:
        st.warning("未检测到已确认的 Single-Factor CFA 结果。请先前往 Single-Factor CFA → 批量自动删题模式完成分析并点击「确认采用」。")
        return

    st.subheader("📋 已确认的 Single-Factor CFA Measure")
    confirmed_df_data = []
    for m_name, payload in confirmed_measures.items():
        final = payload['final']
        confirmed_df_data.append({
            "Measure": m_name,
            ...
            "题目示例": ", ".join(final['items'][:5]) + ("..." if len(final['items']) > 5 else ""),
        })
    st.dataframe(pd.DataFrame(confirmed_df_data), use_container_width=True, hide_index=True)

    # ==========================================================
    # 2. Measure Group 管理
    # ==========================================================
    st.markdown("---")
    st.subheader("🗂️ Measure Group 配置")

    if "batch_multi_groups" not in st.session_state:
        st.session_state.batch_multi_groups = []

    # ---- 2.1 新建 Group ----
    with st.expander("➕ 新建 Measure Group", expanded=len(st.session_state.batch_multi_groups) == 0):
        mg_name = st.text_input(
            "Measure Group 名称（如：核心胜任力组、心理健康组）",
            key="batch_multi_new_name",
            placeholder="请输入 Group 名称",
        )
        available_measure_names = list(confirmed_measures.keys())
        selected_measures = st.multiselect(
            "选择要包含的 Measure（至少 2 个，每个 Measure 对应一个潜变量）",
            options=available_measure_names,
            key="batch_multi_new_measures",
        )
        c1, c2 = st.columns(2)
        with c1:
            N_multi = st.number_input(
                "N_multi（每个 Measure 最小保留题数底线）",
                min_value=3, max_value=50, value=8, step=1,
                help="当某 Measure 剩余题数 ≤ 此值时，该维度题目禁止删除。", key="batch_multi_new_N",
            )
        with c2:
            n_multi = st.number_input(
                "n_multi（每个 Measure 最小保留反向题数底线）",
                min_value=0, max_value=20, value=3, step=1,
                help="当某 Measure 剩余反向题数 ≤ 此值时，该维度反向题禁止删除。", key="batch_multi_new_n",
            )

        if st.button("添加 Measure Group", key="batch_multi_add_group"):
            if not mg_name or not mg_name.strip():
                st.error("请输入 Group 名称。")
            elif len(selected_measures) < 2:
                st.error("Multi-Factor 模型需要至少 2 个潜变量，请选择至少 2 个 Measure。")
            elif any(g['name'] == mg_name.strip() for g in st.session_state.batch_multi_groups):
                st.error("该 Group 名称已存在，请使用其他名称。")
            else:
                st.session_state.batch_multi_groups.append({
                    'id': f"mg_{len(st.session_state.batch_multi_groups)}",
                    'name': mg_name.strip(),
                    'measures': list(selected_measures),
                    'N_multi': int(N_multi),
                    'n_multi': int(n_multi),
                })
                st.success(f"已创建 Group：**{mg_name.strip()}**，包含 {len(selected_measures)} 个 Measure。")
                st.rerun()

    # ---- 2.2 已创建 Group 列表 ----
    if not st.session_state.batch_multi_groups:
        st.info("请至少创建一个 Measure Group 后再继续。")
        return

    st.markdown("##### 已创建的 Measure Group")
    for i, group in enumerate(st.session_state.batch_multi_groups):
        col1, col2 = st.columns([4, 1])
        with col1:
            st.markdown(
                f"**{i+1}. {group['name']}**  "
                f"（{len(group['measures'])} 个 Measure | N_multi={group['N_multi']} | n_multi={group['n_multi']}）"
            )
            st.caption(f"包含 Measure：{', '.join(group['measures'])}")
        with col2:
            if st.button("🗑️ 删除", key=f"batch_multi_del_{group['id']}"):
                st.session_state.batch_multi_groups.pop(i)
                # 清理相关缓存
                for k in list(st.session_state.keys()):
                    if k.startswith(f"batch_multi_res_{group['id']}_") or k.startswith(f"batch_multi_conf_{group['id']}_"):
                        del st.session_state[k]
                st.rerun()
        st.markdown("---")
    ...
    if not has_sub_datasets and not has_dual_data:
        st.warning("未检测到可用的数据源。请先完成数据清洗。")
        return
    ...
    data_source = st.radio("请选择数据来源:", source_options, horizontal=True, key="batch_multi_data_source")

    available_measures_data = {}

    # ---------- 分支 A：子数据集 ----------
    if data_source == "💾 来自 Data Cleaning（子数据集）":
        for m_name in confirmed_measures.keys():
            if m_name not in st.session_state.sub_datasets:
                continue
            df_sub = st.session_state.sub_datasets[m_name].copy()
            ...
            # 只保留 single CFA 确认的题目
            confirmed_items = confirmed_measures[m_name]['final']['items']
            confirmed_items_clean = [re.sub(r'[^\w\u4e00-\u9fa5]', '_', str(c)) for c in confirmed_items]
            cols = [c for c in confirmed_items_clean if c in df_num.columns]
            if cols:
                available_measures_data[m_name] = {'df': df_num[cols], 'items': cols}
        st.session_state.batch_multi_source_type = "sub"
    ...
    # ---------- 分支 B：四数据集 ----------
        ...
        if "batch_multi_dual_dataset" not in st.session_state:
            st.session_state.batch_multi_dual_dataset = dataset_names[1]

        selected_dataset = st.selectbox(
            "选择数据集",
            dataset_names,
            key="batch_multi_dual_dataset"
        )

        all_confirmed_measure_names = list(confirmed_measures.keys())
        df_analysis = get_dual_mode_analysis_df(
            selected_dataset,
            all_confirmed_measure_names,
            st.session_state.dc_dataset_full,
            st.session_state.dc_measures,
            item_columns_only=True,
        )
        ...
        if df_analysis is not None and not df_analysis.empty:
            ...
            for m_name in all_confirmed_measure_names:
                raw_cols = st.session_state.dc_measures.get(m_name, [])
                clean_cols = [re.sub(r'[^\w\u4e00-\u9fa5]', '_', str(c)) for c in raw_cols]
                confirmed_items = confirmed_measures[m_name]['final']['items']
                confirmed_items_clean = [re.sub(r'[^\w\u4e00-\u9fa5]', '_', str(c)) for c in confirmed_items]
                measure_cols = [c for c in clean_cols if c in df_analysis.columns and c in confirmed_items_clean]
                if measure_cols:
                    df_m = df_analysis[measure_cols].apply(pd.to_numeric, errors='coerce').dropna(axis=1, how='all')
                    if not df_m.empty:
                        available_measures_data[m_name] = {'df': df_m, 'items': measure_cols}
        st.session_state.batch_multi_source_type = "dual"
        st.session_state.batch_multi_selected_dual_dataset = selected_dataset

    if not available_measures_data:
        st.error("未能从数据源中获取到任何有效数据，请检查 Data Cleaning 模块。")
        return

    # 验证各 Group 的数据完整性
    all_valid = True
    for group in st.session_state.batch_multi_groups:
        missing = [m for m in group['measures'] if m not in available_measures_data]
        if missing:
            st.warning(f"⚠️ Group「{group['name']}」中的以下 Measure 未找到数据：{', '.join(missing)}")
            all_valid = False
    if not all_valid:
        st.info("请确保所有 Group 中的 Measure 都在当前数据源中存在。")
    ...
    with run_col:
        run_clicked = st.button("🚀 一键批量运行 Multi-Factor 自动删题CFA", type="primary", key="batch_multi_run")
    with prelim_col:
        if "batch_multi_prelim" not in st.session_state:
            st.session_state.batch_multi_prelim = False
        prelim_checked = st.checkbox(
            "当前为 preliminary CFA", value=st.session_state.batch_multi_prelim, key="batch_multi_prelim_checkbox"
        )
        st.session_state.batch_multi_prelim = prelim_checked

    if run_clicked:
        valid_groups = [g for g in st.session_state.batch_multi_groups
                        if all(m in available_measures_data for m in g['measures'])]

        if not valid_groups:
            st.error("没有有效的 Measure Group 可以运行，请检查各 Group 的 Measure 是否都有数据。")
            return
        ...
        for idx, group in enumerate(valid_groups):
            status_text.markdown(f"⏳ **正在计算 ({idx+1}/{len(valid_groups)})**：`{group['name']}` 的 Multi-Factor 自动删题CFA...")

            measure_names = group['measures']

            # 构建合并数据
            merged_df = _build_merged_df_for_group(measure_names, available_measures_data)
            if merged_df is None or merged_df.empty:
                batch_multi_results[group['id']] = {
                    'stage1': None, 'stage2': None, 'error': "数据合并失败或为空"
                }
                continue

            # 构建初始 items_map（从 single CFA 确认结果继承）
            initial_items_map = {}
            initial_method_items = []
            for m in measure_names:
                confirmed_items = confirmed_measures[m]['final']['items']
                clean_items = [re.sub(r'[^\w\u4e00-\u9fa5]', '_', str(c)) for c in confirmed_items]
                valid_items = [c for c in clean_items if c in available_measures_data[m]['items']]
                initial_items_map[m] = valid_items

                # 继承 single CFA 的 method items（若 session 中无记录，则退化为全部反向题）
                single_method = st.session_state.get("batch_measures_config", {}).get(m, {}).get('method_items', [])
                if single_method:
                    valid_method = [c for c in single_method if c in valid_items]
                else:
                    valid_method = [c for c in valid_items if _is_reverse_method_item(c)]
                initial_method_items.extend(valid_method)

            initial_method_items = list(dict.fromkeys(initial_method_items))

            # 约束映射
            N_multi_map = {m: group['N_multi'] for m in measure_names}
            n_multi_map = {m: group['n_multi'] for m in measure_names}

            result = run_auto_multifactor_cfa_two_stages(
                merged_df,
                measure_names,
                initial_items_map,
                initial_method_items,
            ...
            batch_multi_results[group['id']] = result
            progress_bar.progress((idx + 1) / len(valid_groups))

        status_text.success("🎉 所有 Measure Group 的 Multi-Factor 分析完成！")
        st.session_state.batch_multi_results = batch_multi_results
        st.session_state.batch_multi_confirmed = {}
        st.session_state.batch_multi_selected_standard = {}

    # ==========================================================
    # 5. 结果展示
    # ==========================================================
    if 'batch_multi_results' not in st.session_state:
        return

    st.markdown("---")
    st.subheader("📊 分析结果")

    batch_multi_results = st.session_state.batch_multi_results
    if "batch_multi_confirmed" not in st.session_state:
        st.session_state.batch_multi_confirmed = {}
    if "batch_multi_selected_standard" not in st.session_state:
        st.session_state.batch_multi_selected_standard = {}

    # 只展示有结果的 Group
    result_groups = [g for g in st.session_state.batch_multi_groups if g['id'] in batch_multi_results]
    if not result_groups:
        st.info("暂无分析结果可展示。")
        return

    result_tabs = st.tabs([f"📈 {g['name']}" for g in result_groups])

    for tab_idx, group in enumerate(result_groups):
        with result_tabs[tab_idx]:
            res = batch_multi_results.get(group['id'])
            ...
            compare_data = []
            if s1_final:
                measure_details_s1 = " | ".join([
                    f"{m}({s1_final['measure_n_items'].get(m, 0)}题)"
                    for m in group['measures']
                ])
                compare_data.append({
                    "标准": "标准1（保质量）",
                    "总题数": s1_final['total_n_items'],
                    ...
                    "各维度题数": measure_details_s1,
                })
            if s2_final:
                measure_details_s2 = " | ".join([
                    f"{m}({s2_final['measure_n_items'].get(m, 0)}题)"
                    for m in group['measures']
                ])
                compare_data.append({
                    "标准": "标准2（求精简）",
                    "总题数": s2_final['total_n_items'],
                    ...
                    "SRMR": f"{s2_final['srmr']:.3f}" if not np.isnan(s2

...
def _render_dual_tab_preview_and_time(role: str, state_key: str):
    ...
    if st.button("执行批量删除（EFA + CFA）", key="btn_global_remove") and text_remove:
        for role in ["EFA", "CFA"]:
            ...
            rename = {
                c: c.replace(text_remove, "请仔细阅读每个陈述，依据下列 7 点量表（越靠右，符合程度越高），选择一个最符合您实际情况的选项-").strip() or c
                for c in df.columns
...
def _render_one_dataset_cleaning(role: str, state_key: str, item_cols_key: str, assign_efa_after: bool):
    ...
    if st.button(f"执行删除", key=f"btn_remove_{role}") and text_remove:
        rename = {c: c.replace(text_remove, "请仔细阅读每个陈述，依据下列 7 点量表（越靠右，符合程度越高），选择一个最符合您实际情况的选项-").strip() or c for c in df.columns if text_remove in c}
        if rename:
            # 执行重命名并立即进行“重名保险”检查

...
def _render_dual_mode_cleaning():
    ...
    if tab_choice == "EFA 清洗":
        ...
        if not st.session_state.get("dc_merge_done"):
            ...
            # 初始化
            # =====================================================
            if "dc_measures" not in st.session_state:
                st.session_state.dc_measures = {}

            if "dc_measure_groups" not in st.session_state:
                st.session_state.dc_measure_groups = {}
            ...
            if st.button(
                ...
                if not measure_name.strip():
                    ...
                    st.success(
                        f"已添加 Measure「{measure_name}」，共 {len(selected)} 题。"
                    )
            ...
            if st.session_state.dc_measures:
                ...
                del_m = st.selectbox(
                    "删除 Measure",
                    ["(不删除)"] +
                    list(st.session_state.dc_measures.keys()),
                    key="dc_del_measure"
                )
                ...
                if (
                    ...
                    # 自动从 Group 中同步删除
                    for g in st.session_state.dc_measure_groups:

                        st.session_state.dc_measure_groups[g] = [
                            x
                            for x in st.session_state.dc_measure_groups[g]
                            if x != del_m
                        ]
            ...
            # =====================================================
            st.subheader(
                "Measure Group 划分（复杂 Construct）"
            )

            st.caption(
                "将多个 Measure 组合为一个更高阶的 Measure Group。"
            )

            available_measures = list(
                st.session_state.dc_measures.keys()
            )
            ...
            if not available_measures:
                ...
                selected_measures = st.multiselect(
                    "选择组成该 Group 的 Measure",
                    options=available_measures,
                    key="dc_group_measures"
                )

                if st.button(
                    "添加 Measure Group",
                    key="btn_add_measure_group"
                ):

                    if not group_name.strip():

                        st.error(
                            "请输入 Measure Group 名称"
                        )
                        ...
                        st.success(
                            f"已创建 Measure Group「{group_name}」，"
                            f"包含 {len(selected_measures)} 个 Measure。"
                        )

            # =====================================================
            # 显示已创建 Group
            # =====================================================
            if st.session_state.dc_measure_groups:
                ...
                for g, ms in (
                    st.session_state.dc_measure_groups.items()
                ):
                ...
                del_group = st.selectbox(
                    ...
                    ["(不删除)"] +
                    list(
                        st.session_state.dc_measure_groups.keys()
                    ),
                    key="dc_del_group"
                )
                ...
                if (
                    ...
                    del st.session_state.dc_measure_groups[
                        del_group
                    ]
            ...
            if st.session_state.dc_measure_groups:
                ...
                for g, ms in (
                    ...
                    for m in ms:
                        ...
                        item_count = len(
                            st.session_state.dc_measures.get(
                                m,
                                []
                            )
                        )
...
def _render_dual_tab_preview_and_time(role: str, state_key: str):
    """双数据集模式下 EFA/CFA 页签：仅保留数据预览与反应时长筛选，批量删字（同步）、一致性筛选。用 @st.fragment 隔离 widget 交互。"""
    ...
    st.markdown("###### 批量删除指定文字（EFA + CFA 同步）")

    DEFAULT_TEXT = "请仔细阅读每个陈述，依据下列 7 点量表（越靠右，符合程度越高），选择一个最符合您实际情况的选项-"

    text_remove = st.text_input(
        "输入要删除的文字",
        value=DEFAULT_TEXT,
        key="merge_text_remove_shared"
    )

    if st.button("执行批量删除（EFA + CFA）", key="btn_global_remove") and text_remove:
        for role in ["EFA", "CFA"]:
            state_key = f"df_{role.lower()}_current"  # 按你实际session_state改
            ...
            rename = {
                c: c.replace(text_remove, "").strip() or c
                for c in df.columns
                if text_remove in str(c)
            }

            if rename:
                df = _ensure_unique_columns(df.rename(columns=rename))
                st.session_state[state_key] = df

                # 同步 CFA item cols（如果存在）
                key_item = "dc_cfa_item_cols_for_d2d4"
                if key_item in st.session_state:
                    _cols = df.columns.tolist()
                    st.session_state[key_item] = [
                        rename.get(c, c)
                        for c in st.session_state[key_item]
                        if rename.get(c, c) in _cols
                    ]

        st.success("EFA & CFA 批量删除完成")
        _clear_editor_cache()
        st.rerun()
...
def _render_cleaning_on_merged():
    """在 dc_merged_dataset 上执行：注意力/诚实性、IP、一致性、反向计分。生成四数据集后写入 dc_dataset_full。"""
...
def _render_one_dataset_cleaning(role: str, state_key: str, item_cols_key: str, assign_efa_after: bool):
    ...
    text_remove = st.text_input(f"批量删除文字（{role}）", value=DEFAULT_TEXT, key=f"text_remove_{role}")
    ...
    with col2:
        ...
        end_col = st.selectbox(
            f"选择最后一个题项列（{role}）",
            options=df.columns.tolist(),
            index=max(start_idx, len(df.columns) - 1),
            key=f"cons_end_{role}"
        )
    ...
    if st.button(f"执行一致性筛选（{role}）", key=f"btn_consistency_{role}"):
        ...
        try:
            start_idx = df_temp.columns.get_loc(start_col)
            end_idx = df_temp.columns.get_loc(end_col)

            if start_idx > end_idx:
                st.error("起始列不能在结束列之后")
                st.stop()

            item_cols = df_temp.columns[start_idx:end_idx + 1].tolist()

            # =========================
            # 数值化
            # =========================
            num_df = df_temp[item_cols].apply(pd.to_numeric, errors="coerce")

            before_count = len(df_temp)

            # =========================
            # straight-lining（核心逻辑）
            # =========================
            non_null_counts = num_df.notna().sum(axis=1)

            mode_vals = num_df.mode(axis=1).iloc[:, 0] if not num_df.empty else None
            mode_counts = num_df.eq(mode_vals, axis=0).sum(axis=1)

            mask_drop = (
                (non_null_counts > 0) &
                ((mode_counts / non_null_counts) >= ratio)
            )
...
def render_data_cleaning():
    ...
    if st.session_state.df_current is not None:
        ...
        text_to_remove = st.text_input("请输入要批量删除的文字片段:", value=DEFAULT_TEXT, key="text_to_remove_cleaning")
        ...
        def _fragment_consistency_check():
            ...
            all_cols = st.session_state.df_current.columns.tolist()
            ...
            if st.button("执行一致性筛选"):
                ...
                num_df = df_temp[selected_cols].select_dtypes(
                    include=[np.number]
                )
                st.info(
                    f"当前检测范围：{selected_cols[0]} ~ "
                    f"{selected_cols[-1]} "
                    f"（共 {len(selected_cols)} 题）"
                )
                non_null_counts = num_df.notna().sum(axis=1)
        ...
        # 8. 拆分 简单问卷 Measure
        # ==========================================
        st.subheader("8. 拆分 简单问卷 Measure (构建子数据集)")

...
from . import data_cleaning_dual as dc_dual
...
def render_data_cleaning():
    ...
    if st.session_state.df_current is not None:
        ...
        # 9. (新增) 子数据集序号重命名
        # ==========================================
        st.subheader("9. 子数据集序号重命名")
        st.info("对已创建的子数据集进行序号重命名，便于后续分析时使用连续的题目编号。")

        # 创建状态反馈容器
        sub_renumber_status = st.empty()
        ...
        if len(st.session_state.sub_datasets) > 0:
            ...
            selected_dataset = st.selectbox(
                "选择要重命名的子数据集:",
                dataset_names,
                key="sub_dataset_select"
            )

            if selected_dataset:
                sub_df = st.session_state.sub_datasets[selected_dataset]
                sub_cols = sub_df.columns.tolist()

                st.markdown(f"**当前子数据集信息：** {len(sub_cols)} 个题目，{len(sub_df)} 行数据")

                # 显示当前题目列表
                with st.expander("查看当前题目列表", expanded=False):
                    st.write("当前题目：")
                ...
                with col1:
                    auto_renumber = st.checkbox("自动重新编号 (1,2,3...)", value=True, key="auto_renumber_sub")

                if auto_renumber:
                    if st.button("🔄 执行自动序号重命名", key="btn_auto_renumber_sub"):
                        try:
                            # 创建新的列名映射
                            rename_map = {}
                            for i, old_col in enumerate(sub_cols, 1):
                                # 使用正则表达式提取原序号和后面的内容
                                import re
                                match = re.match(r'^(\d+)[_\-\.\:：]?\s*(.+)$', old_col)
                                if match:
                                    # 如果有原序号，替换为新序号
                                    original_num, rest = match.groups()
                                    new_col = f"{i}_{rest}"
                                else:
                                    # 如果没有序号，直接添加新序号
                                    new_col = f"{i}_{old_col}"

                                rename_map[old_col] = new_col

                            # 应用重命名到子数据集
                            st.session_state.sub_datasets[selected_dataset] = _ensure_unique_columns(sub_df.rename(columns=rename_map))

                            # 确保子数据集的更新时间戳
                            if 'sub_datasets_updated' not in st.session_state:
                                st.session_state.sub_datasets_updated = {}
                            st.session_state.sub_datasets_updated[selected_dataset] = pd.Timestamp.now()

                            # 显示成功信息和对照表
                            st.success(f"✅ 成功重新编号子数据集 '{selected_dataset}' 中的 {len(sub_cols)} 个题目！")

                            history_data = [{"原始题目名称": old, "新题目名称": new} for old, new in rename_map.items()]
                            st.write("📋 **重命名对照表：**")
                            st.dataframe(pd.DataFrame(history_data), use_container_width=True)

                            # 在按钮下方显示持久状态反馈
                            sub_renumber_status.info(f"🔄 已重新编号子数据集，共处理了 {len(sub_cols)} 个题目（实时保存完成）")

                        except Exception as e:
                            st.error(f"子数据集序号重命名失败: {e}")
                            # 在按钮下方显示错误状态反馈
                            sub_renumber_status.error("❌ 序号重命名操作失败")
                else:
                    # 手动设置起始序号
                    start_num = st.number_input("设置新的起始序号:", min_value=1, value=1, step=1, key="manual_start_num_sub")

                    if st.button("🔄 执行手动序号重命名", key="btn_manual_renumber_sub"):
                        try:
                            # 创建新的列名映射
                            rename_map = {}
                            current_num = start_num
                            for old_col in sub_cols:
                                # 使用正则表达式提取原序号和后面的内容
                                import re
                                match = re.match(r'^(\d+)[_\-\.\:：]?\s*(.+)$', old_col)
                                if match:
                                    # 如果有原序号，替换为新序号
                                    original_num, rest = match.groups()
                                    new_col = f"{current_num}_{rest}"
                                else:
                                    # 如果没有序号，直接添加新序号
                                    new_col = f"{current_num}_{old_col}"
                            ...
                            # 在按钮下方显示持久状态反馈
                            sub_renumber_status.info(f"🔄 已重新编号子数据集，共处理了 {len(sub_cols)} 个题目（{start_num} 到 {current_num - 1}，实时保存完成）")
            ...
            st.warning("请先创建子数据集，然后再进行序号重命名。")
